...
import classes from './workslistalt.module.css'

import { useState } from 'react'
import paper from 'paper/dist/paper-core'
import { useSelector } from 'react-redux'

const WorksListAlt = React.memo( ({ filter_by_cat }) => {

    const filterClass = filter_by_cat.replace(/[^a-zA-Z]/g, '').toLowerCase()
    ...
    const parentClass = classes.ProjectsListByCat
    ...
    useEffect( () => {
        ...
        const drawRectangles = () => {
            ...
            let leftOffsetAdjust = 0
            if( window.innerWidth >= 1550 ) {
                containerPaddingTop = 168
                leftOffsetAdjust = ( window.innerWidth - 1550 ) / 2
            } else if( window.innerWidth <= 1240 ) {
                containerPaddingTop = 128
            }

            let segemtPositionIndex = 0
            const gridColumns = container.querySelectorAll(`.${filterClass}`)
            ...
            gridColumns.forEach(( col, index ) => {
                ...
                for( let k = 0; k < images.length; k++ ) {
                    ...
                    console.log( leftOffsetAdjust, window.innerWidth )
                    const x = leftOffsetAdjust ? ( img.left - leftOffsetAdjust + (  window.innerWidth * 0.06 ) + 6 - 105 - wraps[k].getBoundingClientRect().width ) : ( img.left - wraps[k].getBoundingClientRect().width - (  window.innerWidth * 0.06 ) + 4 )
                    const y = topOffset
...
*/
    ...
    useEffect( () => {
        ...
        if ( status === 'succeeded' ) {
            const filteredList = works.filter( work => ( work.tag_names.includes( filter_by_cat ) && work.acf.show_in_works === 'Show' ) )
            const priorityList = filteredList.filter( work => work.acf.show_on_top  )
            let listToShow = []
            if( filteredList.length > 2 ) {
                switch( priorityList.length ) {
                    case 0 : listToShow[0] = filteredList[0]; listToShow[1] = filteredList[1]; break;
                    case 1 : listToShow[0] = priorityList[0]; listToShow[1] = filteredList[1]; break;
                    default: listToShow[0] = priorityList[0]; listToShow[1] = priorityList[1]; break;
                }
            } else {
        ...
        <>
            {
                workList.length ? ( 
                    <>
                        <span className={`${classes.servicesWorkList_span }`} >Some of the teams work</span>
                        <section ref={containerRef} className={ `${parentClass}` } >
                            <Masonry
                                breakpointCols={breakpointColumnsObj}
                                className="my-masonry-grid"
                                columnClassName={filterClass}
                            >
                            {
                                ( status === 'succeeded' ) && workList.map( ( work, index ) => {
                                    return (
                                        <div key={ `${filterClass}${index}` } >
                                            {
                                                work ? (
                                                    <div key={work.id}>
                    ...
                    <section ref={containerRef} className={ `${parentClass} pb-5 mb-5` } >
                        <canvas ref={canvasRef} className={ `${ classes.worksCanvas }` } />
                    </section>
                )
            }
